...
// Setup: seed reference data used throughout the test
export function setup() {
  ...
  for (let i = 0; i < 20; i++) {
    const res = http.post(`${BASE_URL}/users`,
      JSON.stringify({ name: `User ${i}`, email: `user${i}@k6test.com` }),
      { headers: HEADERS },
    );
...
export default function ({ users, products }) {
  ...
  if (r < 0.30) {
    ...
    // POST order — 15% (most expensive: transaction + multiple queries)
