...
// Estágio 2: todo o tráfego passa pelo NGINX (porta 8080)
...
export default function ({ users, products }) {
  ...
  if (r < 0.30) {
    // GET user — atravessa NGINX → users-service (hop extra vs monolito)
    ...
    // LIST products — NGINX → monolith-partial
    const res = http.get(`${BASE_URL}/products`);
    check(res, { 'list products 200': (r) => r.status === 200 });

  } else if (r < 0.65) {
    // GET product — NGINX → monolith-partial
    ...
    // POST order — NGINX → monolith-partial (consulta DB de users diretamente)
